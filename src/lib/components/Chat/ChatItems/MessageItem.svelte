        ...
        <div id="output-text-{chatItem.id}-{index}" class="assistant-message-content-part">
          ...
          {#if contentPart.annotations && contentPart.annotations.length > 0}
            <div class="citations">
              ...
              <ol>
                {#each contentPart.annotations as annotation}
                  <li><a href={annotation.url} target="_blank" rel="noopener noreferrer">{annotation.title}</a></li>
                {/each}
              </ol>
            </div>
          {/if}
